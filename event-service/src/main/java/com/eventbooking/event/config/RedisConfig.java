...
public class RedisConfig {
    ...
    public CacheManager cacheManager(RedisConnectionFactory connectionFactory) {
        // Configure ObjectMapper for Redis serialization with JavaTimeModule
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        objectMapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);
        objectMapper.activateDefaultTyping(
        ...
        // Venues cache - 6 hours TTL
        cacheConfigurations.put("venues", defaultConfig.entryTtl(Duration.ofHours(6)));

        // Event availability cache - 5 minutes TTL (frequently updated)
